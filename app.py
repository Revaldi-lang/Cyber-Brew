...
# Rate limiting for login attempts (Format: { username: { 'attempts': int, 'lockout_until': float } })
login_attempts = {}
...
def init_db():
    # Always create database file and tables if not exists
    ...
    customer_record = cursor.execute('SELECT * FROM users WHERE username = ?', ('customer',)).fetchone()
    ...
    if not admin_record:
        p_admin = admin_pass_bcrypt if config.SECURITY_MODE else "admin_123"
        cursor.execute('INSERT INTO users (username, password, role) VALUES (?, ?, ?)', ('admin', p_admin, 'admin'))
    if not customer_record:
        p_cust = customer_pass_bcrypt if config.SECURITY_MODE else "customer_123"
        cursor.execute('INSERT INTO users (username, password, role) VALUES (?, ?, ?)', ('customer', p_cust, 'user'))
        
    # Auto-migration check: convert passwords in DB dynamically based on SECURITY_MODE
    admin_record = cursor.execute('SELECT * FROM users WHERE username = ?', ('admin',)).fetchone()
    customer_record = cursor.execute('SELECT * FROM users WHERE username = ?', ('customer',)).fetchone()
    
    if admin_record and customer_record:
        admin_pass = admin_record['password']
        customer_pass = customer_record['password']
        
        if config.SECURITY_MODE:
            # If SECURITY_MODE is True and passwords are not hashed, convert them to hashes!
            if not admin_pass.startswith('$2b$'):
                print("[Migration] Hashing admin password in DB...")
                cursor.execute('UPDATE users SET password = ? WHERE username = ?', (admin_pass_bcrypt, 'admin'))
            if not customer_pass.startswith('$2b$'):
                print("[Migration] Hashing customer password in DB...")
                cursor.execute('UPDATE users SET password = ? WHERE username = ?', (customer_pass_bcrypt, 'customer'))
        else:
            # If SECURITY_MODE is False and passwords are hashed, convert them to plaintext!
            if admin_pass.startswith('$2b$'):
                print("[Migration] Converting admin password to plaintext in DB...")
                cursor.execute('UPDATE users SET password = ? WHERE username = ?', ('admin_123', 'admin'))
            if customer_pass.startswith('$2b$'):
                print("[Migration] Converting customer password to plaintext in DB...")
                cursor.execute('UPDATE users SET password = ? WHERE username = ?', ('customer_123', 'customer'))
